...
int main(int argc, char * argv[]){
  ...
  for (it=R.begin(); it!=R.end();++it){
	...
	cout<<(*ip).GetPais()<<" ";
	int x =  (int) ((I.num_cols()/360.0) * (180 + pto.getLon()));
	int y =  (int) ((I.num_filas()/180.0) * (90 - pto.getLat()));
	if (ip_before!=Pses.end() ){
	  
	  int x_old =(int) ((I.num_cols()/360.0) * (180 + (*it_before).getLon()));
	  int y_old =  (int) ((I.num_filas()/180.0) * (90 -(*it_before).getLat()));
